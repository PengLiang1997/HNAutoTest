...
class 全局搜索工作区(page):
    ...
    def 文件搜索(self):
        ...
        if not self.wait(全局搜索对象库.文件路径.format("素材4.png","全局搜索1/一级目录"),3) or\
            not self.wait(全局搜索对象库.文件路径.format("素材4.png","全局搜索1/一级目录2"),3) or\
            not self.wait(全局搜索对象库.文件路径.format("素材4.png","全局搜索2/一级目录"),3):
        ...
        if not self.wait(全局搜索对象库.列表文件名称.format("素材7.png"), 3):
    ...
    def 目录搜索(self):
        ...
        self.全局搜索页面.搜索文件(搜索条件="文件目录21")
        if not self.wait(全局搜索对象库.目录创建人.format("文件目录21", "18942178870"), 3):
            raise AssertionError("输入文件目录名称，未搜索到指定的文件目录")
        # 目录搜索支持目录名称模糊匹配
        self.全局搜索页面.搜索文件(搜索条件="一级目录2")
        if not self.wait(全局搜索对象库.目录创建人.format("文件目录21", "18942178870"), 3)or \
                not self.wait(全局搜索对象库.目录创建人.format("文件目录2", "18942178870"),3):
        ...
        if not self.wait(全局搜索对象库.目录路径.format("一级目录", "局搜索1/一级目录"), 3) or \
    ...
    def 项目成员搜索(self):
        ...
        if not self.wait(全局搜索对象库.成员项目名称.format("18942178871", "全局搜索1"), 3) or \
                not self.wait(全局搜索对象库.项目创建人.format("18942178871", "全局搜索3"), 3):
