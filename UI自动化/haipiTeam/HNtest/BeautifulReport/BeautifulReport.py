...
from HNtest.Secselenium.secdriver import Secdriver as Secdriver
from HNtest.testcasesec.testcasesec import *
...
class PATH:
    """ all file PATH meta """
    config_tmp_path = sys.path[0] + '/HNtest/BeautifulReport/template'
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def output_report(self, theme):
        ...
        with open(template_path+'/template.html', 'rb') as file:
    ...
    def screenshot(self:TestCaseSec):
        try:
            ...
            if isinstance(e, unittest.SkipTest):
                raise
            img_path=self.get_screenshot(funcname=func.__name__)
            print("截图位置是："+img_path)
            data = BeautifulReport.img2base(img_path)
            print(HTML_IMG_TEMPLATE.format(data, data))
            # sys.exit(0)
            raise e
        print('<br></br>')
